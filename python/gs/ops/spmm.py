...
def gspmm(g, op, reduce_op, lhs_data, rhs_data, on_format):
    ...
    return gspmm_internal(g._graph, op, reduce_op, lhs_data, rhs_data, on_format)
...
def _gen_copy_reduce_func(binary_op, reduce_op):
    ...
    def func(g, x, on_format=_CSC):
        if binary_op == "copy_u":
            return gspmm(g, "copy_lhs", reduce_op, x, None, on_format)
        else:
            return gspmm(g, "copy_rhs", reduce_op, None, x, on_format)
...
def _register_spmm_func():
    ...
    - Binary operation plus reduction between node and edge: u_[]_e_[]
