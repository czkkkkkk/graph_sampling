...
ops_better_format = {
    "_CAPI_Slicing": [2, {0:[_CSR, _COO, _CSC], 1:[_CSC, _COO, _CSR]}],
    "_CAPI_Sampling": [1, {0:[_CSR], 1:[_CSC]}],
    "_CAPI_SamplingProbs": [1, {0:[_CSR], 1:[_CSC]}],
    "_CAPI_SDDMM" : [None, [_COO, _CSC, _CSR]],
    "_CAPI_SpMM" : [8, {2:[_CSR, _COO], 0:[_CSC, _COO]}],
    "_CAPI_SlicingSampling" : [1, {0:[_CSR], 1:[_CSC]}],
    "_CAPI_FusedUOPV" : [None, [_COO, _CSC]],
    "_CAPI_FusedESquareSum" : [8, {2:[_CSR, _COO], 0:[_CSC, _COO]}],
    "_CAPI_FusedEDivUSum" : [8, {2:[_CSR, _COO], 0:[_CSC, _COO]}],
}
    ...
    def format_selection_gm(self, gm, args):
        ...
        def bench_plan(plan, gm):
            for node in gm.graph.nodes:
                if node.target in format_candidate_ops:
                    ...
                    available_formats = plan[candidate_graphs_formats[node.args[0]]]
                    
                    # find best format
                    axis, prefer_format_list = ops_better_format[node.target]
                    if axis is not None:
                        prefer_format_list = prefer_format_list[node.args[axis]]
        ...
        for node in gm.graph.nodes:
            if node.target in format_candidate_ops:
                ...
                candidate_graphs_formats[node.args[0]].add(node.args[on_format_index])
