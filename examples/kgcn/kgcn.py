...
def kgcn_sampler(A: gs.matrix_api.Matrix, seeds: torch.Tensor, fanouts: List):
    input_node = seeds
    ret = []
    for K in fanouts:
        subA = A[:, seeds]
        sampleA = subA.individual_sampling(K, None, True)
        seeds = sampleA.csc[1]
        ret.append(sampleA)
    output_node = seeds
...
if __name__ == "__main__":
    ...
    #    func=graphsage_sampler, args=(m, seeds, [25, 10]))
    compile_func = kgcn_sampler
