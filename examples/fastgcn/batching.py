...
# 1. 等价的大batchsize的实验 （GraphSAGE)
# 2. encoding FastGCN (https://github.com/czkkkkkk/graph_sampling/blob/dev_batching_ops/examples/fastgcn/batching_encoding.py)
# 3. BatchConcat + BatchRelabelByKey + BatchSplit -> BatchCOORelabel{ByKey}

torch.manual_seed(1)

g, features, labels, n_classes, splitted_idx = load_ogb(
    'ogbn-products', root='/home/ubuntu/.dgl')
    ...
    for it, seeds in enumerate(tqdm(seedloader)):
        ...
        for layer, fanout in enumerate(fanouts):
            ...
            neighbors_ptr = torch.ops.gs_ops.GetBatchOffsets(
                neighbors_batch_mask, num_batchs)
            ...
            sub_coo_ptr = torch.ops.gs_ops.GetBatchOffsets(
                sub_coo_row_batch_mask, num_batchs)
