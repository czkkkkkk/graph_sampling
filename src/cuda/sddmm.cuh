#ifndef GS_CUDA_SDDMM_CUH_
#define GS_CUDA_SDDMM_CUH_
...
template <typename Idx, typename DType, typename Op, int LhsTarget = 0,
          int RhsTarget = 2>
void SDDMMCoo(const BcastOff& bcast, std::shared_ptr<COO> coo, torch::Tensor lhs,
              torch::Tensor rhs, torch::Tensor out) {
  const Idx* row = coo->row.data_ptr<Idx>();
  const Idx* col = coo->col.data_ptr<Idx>();
  const Idx* edge_map = coo->e_ids.data_ptr<Idx>();
