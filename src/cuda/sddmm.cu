...
//  and
//  * destination node index of an given edge on Csr format, it uses binary
//  search
...
void SDDMMCOOCUDA(const BcastOff& bcast, std::shared_ptr<COO> coo,
  ...
  const int64_t nnz = coo->row.size(0);
  ...
  BCAST_IDX_CTX_SWITCH(bcast, use_idx, lhs_off, rhs_off, {
    CUDA_KERNEL_CALL((SDDMMCooKernel<Idx, DType, Op, UseBcast, UseIdx,
                     ...
                     edge_map, num_rows, num_cols, nnz, reduce_dim, lhs_off,
  ...
  //     UseIdx,
  //                                                LhsTarget, RhsTarget>),
  //                      nblks, nthrs, 0, stream, lhs_data, rhs_data, out_data,
  //                      row, col, edge_map, coo.num_rows, coo.num_cols, nnz,
  //                      reduce_dim, lhs_off, rhs_off, lhs_len, rhs_len, len);
  //   });
  // } else {
  //   const int ntx = FindNumThreads(len);
  //   const int nty = CUDA_MAX_NUM_THREADS / ntx;
...
// template <typename Idx, typename DType, typename Op, int LhsTarget = 0,
//           int RhsTarget = 2>
// void SDDMMCsr(const BcastOff& bcast, const CSRMatrix& csr, torch::Tensor lhs,
...
//   int64_t *lhs_off = nullptr, *rhs_off = nullptr;
//   int64_t len = bcast.out_len, lhs_len = bcast.lhs_len, rhs_len =
//   bcast.rhs_len; int64_t reduce_dim = bcast.reduce_size;
...
//                                      LhsTarget, RhsTarget>),
//                      nblks, nthrs, 0, stream, lhs_data, rhs_data, out_data,
//                      indptr, indices, edge_map, N, M, E, reduce_dim, lhs_off,
//                      rhs_off, lhs_len, rhs_len, len);
//   });
// }
  ...
  SWITCH_BITS(32, DType, {
    SWITCH_OP(op, Op, {
      SWITCH_TARGET(lhs_target, rhs_target, LhsTarget, RhsTarget, {
        SDDMMCOOCUDA<int64_t, DType, Op, LhsTarget, RhsTarget>(
            bcast, coo, lhs, rhs, out, num_rows, num_cols);
      });
    });
  });
}
