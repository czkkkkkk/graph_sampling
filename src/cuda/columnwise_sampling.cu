...
__global__ void _SampleSubIndicesKernelFusedWithReplace(
    ...
    IdType* column_ids, int64_t size, const uint64_t rand_seed) {
  assert(blockDim.x == BLOCK_SIZE);
  int64_t row = blockIdx.x * TILE_SIZE;
  const int64_t last_row =
      min(static_cast<int64_t>(blockIdx.x + 1) * TILE_SIZE, size);
...
torch::Tensor SampleSubIndicesFused(torch::Tensor indptr, torch::Tensor indices,
  ...
  if (replace) {
    ...
    _SampleSubIndicesKernelFusedWithReplace<int64_t, TILE_SIZE>
        <<<grid, block>>>(
            sub_indices.data_ptr<int64_t>(), indptr.data_ptr<int64_t>(),
