...
__global__ void SpMMCSCKernel(
    ...
    const Idx* __restrict__ edge_map, const Idx* __restrict__ nid_map,
    int64_t num_cols, const int64_t* __restrict__ ubcast_off,
    const int64_t* __restrict__ ebcast_off, int64_t ufeat_len,
    int64_t efeat_len, int64_t out_len) {
  // SPMM with CSC.
  ...
  while (ty < num_cols) {
    ...
    while (tx < out_len) {
      ...
      int out_pos = UseNMap ? nid_map[ty * out_len + tx] : ty * out_len + tx;
...
void SpMMCSCCUDA(const BcastOff& bcast, const std::shared_ptr<CSC> csc,
                 ...
                 torch::Tensor efeat, torch::Tensor out, torch::Tensor argu,
                 torch::Tensor arge) {
    ...
    ID_TYPE_SWITCH(csc->indices.scalar_type(), IdType, {
      ...
      SWITCH_OP(op, Op, {
        SpMMCSCCUDA<IdType, DType, Op, impl::reduce::Sum<IdType, DType> >(
            bcast, csc, n_ids, ufeat, efeat, out, out_aux[0], out_aux[1]);
      });
    });
  } else if (reduce == "max") {
    ID_TYPE_SWITCH(csc->indices.scalar_type(), IdType, {
    ...
    ID_TYPE_SWITCH(coo->row.scalar_type(), IdType, {
      ...
      SWITCH_OP(op, Op, {
        SpMMCOOCUDA<IdType, DType, Op, impl::reduce::Sum<IdType, DType, true> >(
            bcast, coo, ufeat, efeat, out, torch::Tensor(), torch::Tensor());
      });
    });
  } else if (reduce == "max") {
    ID_TYPE_SWITCH(coo->row.scalar_type(), IdType, {
