...
__global__
void _GetSubSizeKernel(int64_t* sub_indptr, int64_t* indptr, int64_t* column_ids, int64_t size) {
...
torch::Tensor GetSubIndptr(torch::Tensor indptr, torch::Tensor ids) {
  auto topt = torch::TensorOptions().dtype(indptr.dtype()).device(indptr.device());
  ...
  _GetSubSizeKernel<<<n_blocks, n_threads>>>(sub_size.data_ptr<int64_t>(), indptr.data_ptr<int64_t>(), ids.data_ptr<int64_t>(), size);
  return sub_size.cumsum(0);
}

__global__
void _GetSubIndicesKernel(int64_t* out_indices, int64_t* indptr, int64_t* indices, int64_t* sub_indptr, int64_t* column_ids, int64_t size) {
...
torch::Tensor GetSubIndices(torch::Tensor indptr, torch::Tensor indices, torch::Tensor sub_indptr, torch::Tensor column_ids) {
  ...
  _GetSubIndicesKernel<<<grid, block>>>(sub_indices.data_ptr<int64_t>(), indptr.data_ptr<int64_t>(), indices.data_ptr<int64_t>(), sub_indptr.data_ptr<int64_t>(), column_ids.data_ptr<int64_t>(), size);
  return sub_indices;
}
