...
namespace fusion {
 ...
 __device__ bool BinarySearch(int64_t *ptr, int64_t degree, int64_t target) {
  ...
  __device__ bool CheckConnect(int64_t* graph_indice, int64_t* graph_indptr,int64_t degree, int64_t src, int64_t dst) {
      ...
      if (curr == -1) {
        ...
        do {
          ...
          out_traces_data[(step_idx + 1) * num_seeds + idx] = outV;
            ...
            out_traces_data[(step_idx + 1) * num_seeds + idx] = graph_indice[in_row_start];
        } 
        else {
...
torch::Tensor FusedNode2VecCUDA(torch::Tensor seeds, int64_t walk_length,
                                  int64_t* indices, int64_t* indptr,double p,double q) {
