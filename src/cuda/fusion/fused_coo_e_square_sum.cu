...
__global__ void COOESquareUSumKernel(
  ...
  while (tx < E) {
    ...
    const DType* uoff = BinaryOp::use_lhs ? (ufeat + dst * ufeat_len) : nullptr;
    const DType* eoff = BinaryOp::use_rhs ? (efeat + eid * efeat_len) : nullptr;
    DType* outoff = out + src * out_len;
    DType val = BinaryOp::Call(uoff,eoff);
    val = val * val;
...
template <typename Idx, typename DType, typename BinaryOp, typename ReduceOp>
void  COOESquareSumCUDA(const BcastOff& bcast, const std::shared_ptr<COO> coo,
  ...
  const dim3 nthrs(ntx);
      ...
      CUDA_KERNEL_CALL(
          (COOESquareUSumKernel<Idx, DType, BinaryOp, ReduceOp, false, false>),
...
void ESquareSumCOO(const std::string& op, const std::string& reduce,
  ...
  if (reduce == "sum") {
    ID_TYPE_SWITCH(coo->row.scalar_type(), IdType, {
      typedef float DType;
      SWITCH_OP(op, Op, {
        COOESquareSumCUDA<IdType, DType, Op, impl::reduce::Sum<IdType, DType> >(
