...
inline __device__ float AtomicMax(
  float * const address,
  const float val
){
  float old;
...
inline __device__ uint64_t AtomicCAS(
...
inline __device__ uint32_t AtomicCAS(
...
inline __device__ int64_t AtomicMin(
    int64_t * const address,
    const int64_t val) {
  // match the type of "::atomicCAS", so ignore lint warning
  using Type = long long int; // NOLINT
  ...
  return atomicMin(reinterpret_cast<Type*>(address),
                   static_cast<Type>(val));
}


inline __device__ uint64_t AtomicMin(
    uint64_t * const address,
    const uint64_t val) {
  // match the type of "::atomicCAS", so ignore lint warning
  using Type = unsigned long long int; // NOLINT
...
inline __device__ int32_t AtomicMin(
    int32_t * const address,
    const int32_t val) {
  // match the type of "::atomicCAS", so ignore lint warning
  using Type = int; // NOLINT
...
inline __device__ uint32_t AtomicMin(
    uint32_t * const address,
    const uint32_t val) {
  // match the type of "::atomicCAS", so ignore lint warning
...
inline __device__ float AtomicMin(
  float * const address,
  const float val
){
  float old;
...
inline __device__ int32_t AtomicAdd(
  int32_t * const address,
  const int32_t val) {
    using Type = int; // NOLINT

    static_assert(sizeof(Type) == sizeof(*address), "Type width must match");
    return atomicAdd(reinterpret_cast<Type*>(address),
                   static_cast<Type>(val));
}
...
inline __device__ float AtomicAdd(
  float * const address,
  const float val) {
...
inline __device__ int32_t AtomicSub(
  int32_t * const address,
  const int32_t val) {
    using Type = int; // NOLINT

    static_assert(sizeof(Type) == sizeof(*address), "Type width must match");
    return atomicSub(reinterpret_cast<Type*>(address),
                   static_cast<Type>(val));
}


inline __device__ int64_t AtomicSub(
  int64_t * const address,
  const int64_t val) {
...
inline __device__ float AtomicSub(
  float * const address,
  const float val) {
    return AtomicAdd(address, (-val));
}
