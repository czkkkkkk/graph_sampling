...
BatchCOORowSlicing(std::shared_ptr<COO> coo, torch::Tensor node_ids,
  ...
  if (coo->col.device().type() == torch::kCUDA) {
    torch::Tensor sub_coo_row, sub_coo_col, coo_ptr;
    std::tie(sub_coo_row, sub_coo_col, coo_ptr) = impl::BatchCOORowSlicingCUDA(
        coo->row, coo->col, node_ids, indices_ptr, node_ids_ptr);
    return std::make_tuple(
        std::make_shared<COO>(COO{sub_coo_row, sub_coo_col, torch::nullopt,
                                  coo->row_sorted, coo->col_sorted}),
        sub_coo_row, sub_coo_col, coo_ptr);
  } else {
    LOG(FATAL) << "Not implemented warning";
